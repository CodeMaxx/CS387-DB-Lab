...
public class Login extends HttpServlet {
	private static final long serialVersionUID = 1L;
      static String host = "jdbc:postgresql://localhost:5310/postgres";
      static String username = "atrehan";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ID !=null && pass != null) {
			try (
				    ...
				    PreparedStatement stmt = conn.prepareStatement("select * from password where ID=? and password=?");
				){
				stmt.setString(1, ID);
				stmt.setString(2, pass);
				ResultSet rs = stmt.executeQuery();PrintWriter out = response.getWriter();
